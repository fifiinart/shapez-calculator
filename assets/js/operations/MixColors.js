...
export const MixColors = createOperation({
    name: "MixColors",
    ingredients: ["color", "color"],
    result: "color",
    operate([color1, color2]) {
        if (color1 === Color.uncolored || color2 === Color.uncolored)
            throw new OperationInvalidIngredientError("Neither color can be uncolored in a MixColors operation");
        for (const [[ingredient1, ingredient2], result] of ColorMixingRecipes) {
            if ((ingredient1 === color1 && ingredient2 === color2) || (ingredient1 === color2 && ingredient2 === color1))
                return result;
        }
